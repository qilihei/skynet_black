    ...
    execute: function () {
      ...
      _export("LoginScene", LoginScene = (_dec = ccclass('LoginScene'), _dec2 = property(EditBox), _dec3 = property(EditBox), _dec4 = property(Button), _dec5 = property(Button), _dec6 = property(Label), _dec7 = property(Label), _dec8 = property(Node), _dec(_class = (_class2 = class LoginScene extends Component {
        ...
        updateUIState() {
          ...
          const isConnected = connectionState === (_crd && ConnectionState === void 0 ? (_reportPossibleCrUseOfConnectionState({
            error: Error()
        ...
        updateStatusLabel(message, isError = false) {
          if (this.statusLabel) {
        ...
        onLoginButtonClick() {
          ...
          const username = (_this$usernameInput = this.usernameInput) == null || (_this$usernameInput = _this$usernameInput.string) == null ? void 0 : _this$usernameInput.trim();
          const password = (_this$passwordInput = this.passwordInput) == null || (_this$passwordInput = _this$passwordInput.string) == null ? void 0 : _this$passwordInput.trim();
        ...
        async login(username, password) {
          ...
          try {
            ...
            const cmdId = this.protocolManager.getCommandId('LOGIN_REQUEST'); // 发送登录请求

            const response = await this.networkManager.sendRequest(cmdId, loginData, 10000);

            if (response.code === 0) {
              // 登录成功
              const loginResponse = this.protocolManager.decodeLoginResponse(response.data); // 保存登录数据

              this.gameDataManager.setSessionToken(loginResponse.session_token);
              this.gameDataManager.setPlayerData(loginResponse.player_info); // 保存登录信息到本地

              this.saveLoginInfo(username);
              this.updateStatusLabel('登录成功，正在进入游戏...'); // 延迟一下再跳转，让用户看到成功消息
